    ...
    <div>
      <swiper :options="swiperOption" ref="swiperOption">
        <swiper-slide class="swiper-slide">
          <div class="title">{{school_name}}综合排名变化情况</div>
          <bar :ranking='ranking' :schoolName='school_name'/>
        </swiper-slide>
        <swiper-slide class="swiper-slide">
          <div class="title">{{school_name}}综合排名变化情况</div>
          <combination />
...
import combination from "../echarts/combination.vue";
...
export default {
  ...
  watch: {
    schoolInfo: function (newVal, oldVal) {
      console.log('测试123456',newVal)
      this.ranking = newVal.ranking;
      this.school_name=newVal.introduction.name_cn||newVal.introduction.name
      console.log("ranking", newVal);
    },
  },
  data() {
    return {
